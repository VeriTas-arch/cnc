...
def run_syn_GJ(syn_model, title, run_duration=100.0, Iext=7.5, **kwargs):
    # 定义神经元组和突触连接，并构建神经网络
    neu = bp.neurons.HH(
        2, V_initializer=bp.init.Constant(-70.68)
    )  # bm.array([-60,-70.68])  bm.array([40,-70.68])
    syn = syn_model(neu, neu, conn=bp.connect.All2All(include_self=False), **kwargs)
    net = bp.Network(syn=syn, neu=neu)  # include_self=False: 自己和自己没有连接
    # 运行模拟
    runner = bp.DSRunner(
        net,
...
run_syn_GJ(GapJunction, Iext=5.0, title="Gap Junction Model")  # 20
